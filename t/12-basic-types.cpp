...
TEST_CASE("ping", "[connection]") {
    ...
    std::vector<read_callback_t> callbacks {
        [&](const boost::system::error_code &error_code, r::redis_result_t &&r, size_t consumed) {
    ...
    read_callback_t generic_callback = [&](const boost::system::error_code &error_code, r::redis_result_t &&r, size_t consumed){
       REQUIRE(!error_code);
    ...
    c.async_write(r::command_wrapper_t(cmds_container), [&](const auto &error_code) {
       REQUIRE(!error_code);
       c.async_read(rx_buff, generic_callback);
    });
